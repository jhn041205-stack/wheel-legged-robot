function K = get_k_length(leg_length)
    ...
    R1=0.13;                         %驱动轮半径
    nl = -147.93*leg_length^5 + 247.3*leg_length^4 - 168.27*leg_length^3 + 58.992*leg_length^2 - 10.938*leg_length + 1.2942;
    L1=leg_length * nl;                  %摆杆重心到驱动轮轴距离
    LM1=leg_length - L1;                 %摆杆重心到其转轴距离
    l1=3.5e-3;                          %机体质心距离转轴距离
    mw1=0.81;                         %驱动轮质量
    mp1=2.125;                         %杆质量
    M1=7;                           %机体质量
    Iw1=6265.163e-6;                     %驱动轮转动惯量
    Ip1=0.1117*leg_length^5 - 0.2653*leg_length^4 + 0.2135*leg_length^3 + 0.1354*leg_length^2 + 0.0195*leg_length + 0.0241; %摆杆转动惯量
    %IM1=M1*(0.3^2+0.12^2)/12.0;       %机体绕质心转动惯量
    IM1=50631.029e-6;       %机体绕质心转动惯量
    ...
    Q=diag([100 1 300 60 1500 1]);%theta d_theta x d_x phi d_phi%700 1 600 200 1000 1
    R=[1 0;0 0.25];                %T Tp [240 0;0 25]
