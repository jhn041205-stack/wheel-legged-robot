...
- 在 [robot_typedef](../application/robot_typedef.h) 中定义不同硬件的类型ID，如：
      ...
      #define CHASSIS_BIPEDAL         4  // 平衡底盘
      ```

- 在**机器人参数配置文件**中选择模块的硬件类型，如：
    
    ```C
    #define CHASSIS_TYPE CHASSIS_BIPEDAL
    ```
...
- 在不同硬件的代码部分使用预编译指令判断是否需要编译（直接用`#if() #endif` 将整个代码部分都包裹起来）
    ...
    #if (CHASSIS_TYPE == CHASSIS_BIPEDAL)
    ...
    2. `#ifndef CHASSIS_BIPEDAL_H` 应当作用于整个文件，所以要写在最上方。
    <details>
